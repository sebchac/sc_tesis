...
\begin{equation}
    \centering
    $ x_{i,t} = \beta_{1} Contrafactual + \beta_{2} Líderes + \beta_{3} Contrafactual \times Líderes + e_{i,t} $
\end{equation}

donde $\hat{x}_{i,t}$ es la variable de interés, que puede referirse a los costos ($\hat{c}_{i,t}$), exportaciones ($q_{i,t}$) o beneficios ($\pi_{i,t}$). Por otro lado, $Contrafactual$ y $Líderes$ son variables dicotómicas que toman el valor 1 si corresponde al escenario contrafactual o a líderes, respectivamente. El coeficiente de interés para este análisis es aquel asociado a la intersección de las variables explicativas anteriores, pues refleja si existe un efecto diferencial entre escenarios por el solo hecho de pertenecer a un determinado grupo exportador.
