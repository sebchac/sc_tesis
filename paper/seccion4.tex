...
En adelante, la metodología propuesta considera que la temperatura evoluciona en el día siguiendo una función triangular, comenzando y terminando con un valor mínimo, y presentando una temperatura máxima en el vértice superior. Aunque sencilla, esta aproximación permite obtener un valor directo para la fracción del día en que se alcanza un nivel de temperatura específico. Así, para un determinado umbral de temperatura $K$, y temperaturas mínima y máxima ($t_{min}$ y $t_{max}$, respectivamente, con $t_{min} < t_{max}$), se tienen tres escenarios posibles, presentados en la Figura N°[]:
