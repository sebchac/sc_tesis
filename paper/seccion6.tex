...
\begin{equation}
    Q_{t} = \alpha^{d}_{0} + \alpha^{d}_{1}P_{t} + \alpha^{d}_{2}X_{t} + \alpha^{d}_{3}Z_{t} + u^{d}_{t}
\end{equation}
...
\begin{equation}
    P_{t} = \frac{1}{\alpha^{d}_{1}} \left[ Q_{t} - ( \alpha^{d}_{0} + \alpha^{d}_{2}X_{t} + \alpha^{d}_{3}Z_{t} + u^{d}_{t} ) \right]
\end{equation}
...
\begin{figure}[H]
    \centering
    \caption{Distribución de la elasticidad-precio de la demanda. Modelo (6)}
        \includegraphics[scale = 0.6]{figures/elasticidad_iv3.png}
        \label{fig:elast_iv3}
\end{figure}
...
\alpha^{o}_{5}Fr_{i, t} +
\alpha^{o}_{6}Fr_{i, t-1} +
\phi_{i}+u^{o}_{i,t}
\end{equation} 

donde $C_{t}$ es una variable de tendencia para identificar un ciclo de producción de 5 años, que es el tiempo en que tarda en madurar un cafeto, $F_{t}$ es el índice de precios de fertilizantes calculado por el Banco Mundial, $H_{i, t}$ es la exposición anual al calor, para cada país, $Fr_{i, t}$ es el equivalente para la exposición al frío, $H_{i, t-1}$ y $Fr_{i, t-1}$ son los rezagos correspondientes, $\phi_{i}$ es un efecto fijo por país, y $u^{o}_{i,t}$ es el término error.
...
Las Figuras N°\ref{fig:plot06_01} y N°\ref{fig:plot06_02} muestran el ajuste de la estimación para el precio y las cantidades observadas, respectivamente. La estimación del modelo Stackelberg presenta un ajuste razonable a los datos observados. Sin embargo, es importante remarcar las divergencias. En el caso del precio, es claro que la estimación tiende a suavizar algunas desviaciones, como los picos en la década de los 90s, o el máximo alcanzado en el año 2012. Por otro lado, sobrestima el alza previa al año 2010, al igual que los valores entre 2000 y 2005. El ajuste en las cantidades tiende a suavizar las desviaciones de tendencia, con una sobrestimación relevante en la década de 1990 y una subestimación desde el año 2015.
