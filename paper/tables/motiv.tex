\begin{table}[H]
\centering
\caption{Efecto de la temperatura en los países productores}
\label{tab:motiv}
\begin{tabular}{lccc}
\toprule
 & \textbf{Cantidad} & \textbf{Ingresos} & \textbf{Participación} \\
 & \textbf{(log)} & \textbf{(log)} & \textbf{de mercado} \\
 & (1) & (2) & (3) \\
\midrule
Temperatura & 0.343*** & 1.015*** & 1.618*** \\
 & (0.0265) & (0.0760) & (0.173) \\
Temperatura$^2$ & -0.00898*** & -0.0274*** & -0.0424*** \\
 & (0.000639) & (0.00184) & (0.00418) \\
Constante & -2.404*** & -4.309*** & -11.99*** \\
 & (0.275) & (0.788) & (1.796) \\
\midrule
Observaciones & 2,532 & 2,532 & 2,532 \\
R$^2$ & 0.0833 & 0.105 & 0.0454 \\
R$^2$ ajustado & 0.0826 & 0.105 & 0.0446 \\
\bottomrule
\multicolumn{4}{p{10cm}}{\footnotesize{\textit{Nota:} Errores estándar entre paréntesis. *** p$<$0.01, ** p$<$0.05, * p$<$0.1.}}
\end{tabular}
\end{table}