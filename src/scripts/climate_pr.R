...
base_path2 <- "/Users/sebastianchacon/Desktop/sc_tesis/src/original_data/pr"
...
process_period <- function(year_start, year_end, coffee_mask, country_cells, cell_mapping) {
  ...
  pr_file <- file.path(base_path2, 
                       sprintf("20crv3-w5e5_obsclim_pr_global_daily_%d_%d.nc", year_start, year_end))
  
  if (!file.exists(pr_file)) {
    cat("ADVERTENCIA: Archivo no encontrado para", year_start, "-", year_end, "\n")
    return(NULL)
  }
  ...
  # Aplicar máscara y convertir a mm/day (de kg/m²/s a mm/day)
  # 1 kg/m²/s = 86400 mm/day
  pr <- mask(pr, coffee_mask) * 86400
  ...
  for(year in year_start:year_end) {
    ...
    pr_year <- values(pr[[year_indices]])[coffee_cells_all, , drop = FALSE]
    ...
    monthly_year <- lapply(unique(country_cells$country), function(country_name) {
      ...
      pr_country <- pr_year[country_positions, , drop = FALSE]
      daily_pr <- colMeans(pr_country, na.rm = TRUE)
      ...
      data.frame(
        ...
        summarise(
          date = min(date),
          pr_ymc = sum(pr_daily, na.rm = TRUE),
          .groups = 'drop'
        )
    })
...
pr_ref <- rast(file.path(base_path2, 
                         sprintf("20crv3-w5e5_obsclim_pr_global_daily_%d_%d.nc", first_period[1], first_period[2])))

coffee_area <- resample(coffee_area, pr_ref, method = "sum")
...
for(i in seq_along(periods)) {
  ...
  if(!is.null(result)) {
    ...
    output_file <- file.path(output_path, 
                             sprintf("df_pr_%d_%d.rds", period[1], period[2]))
...
if(length(all_results) > 0) {
  ...
  output_complete <- file.path(output_path, "df_pr_complete_1961_2019.rds")
