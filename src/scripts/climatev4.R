...
process_period <- function(year_start, year_end, coffee_mask, country_cells, cell_mapping) {
  ...
  for(year in year_start:year_end) {
    ...
    monthly_year <- lapply(unique(country_cells$country), function(country_name) {
      ...
      total_ha_country <- sum(country_cells_info$total_ha)
      
      tmax_country <- tasmax_year[country_positions, , drop = FALSE]
      tmin_country <- tasmin_year[country_positions, , drop = FALSE]
      
      # Inicializar matrices para acumular GDD, HDD, FDD ponderados
      n_days <- ncol(tmax_country)
      n_cells <- nrow(tmax_country)
      
      gdd_weighted <- matrix(0, nrow = n_cells, ncol = n_days)
      hdd_weighted <- matrix(0, nrow = n_cells, ncol = n_days)
      fdd_weighted <- matrix(0, nrow = n_cells, ncol = n_days)
      ...
      for(cell_idx in 1:n_cells) {
        ...
        ha_total_cell <- ha_arabica + ha_robusta
        ...
        # Ponderar por hectáreas
        gdd_weighted[cell_idx, ] <- gdd_cell * ha_total_cell
        fdd_weighted[cell_idx, ] <- fdd_cell * ha_total_cell
        hdd_weighted[cell_idx, ] <- (hdd_arabica * ha_arabica + hdd_robusta * ha_robusta)
      }
      
      # Sumar sobre todas las celdas y dividir por total de hectáreas
      daily_gdd <- colSums(gdd_weighted, na.rm = TRUE) / total_ha_country
      daily_hdd <- colSums(hdd_weighted, na.rm = TRUE) / total_ha_country
      daily_fdd <- colSums(fdd_weighted, na.rm = TRUE) / total_ha_country
      
      # Para tmax y tmin, usar promedio ponderado por hectáreas
      weights_matrix <- matrix(rep(country_cells_info$total_ha, n_days), ncol = n_days)
      daily_tmax <- colSums(tmax_country * weights_matrix, na.rm = TRUE) / sum(country_cells_info$total_ha)
      daily_tmin <- colSums(tmin_country * weights_matrix, na.rm = TRUE) / sum(country_cells_info$total_ha)
      
      # Calcular proporciones
      arabica_prop <- sum(arabica_ha_vec) / total_ha_country
...
for(i in seq_along(periods)) {
  ...
  if(!is.null(result)) {
    ...
    output_file <- file.path(output_path, 
                             sprintf("df_gdd_%d_%d_weighted.rds", period[1], period[2]))
...
if(length(all_results) > 0) {
  ...
  output_complete <- file.path(output_path, "df_gdd_complete_1961_2020_weighted.rds")
  ...
  # Resumen por país
  cat("\nRESUMEN POR PAÍS (primeros 10):\n")
  summary_by_country <- complete_data %>%
    group_by(country) %>%
    summarise(
      total_ha = first(total_ha),
      arabica_prop = first(arabica_proportion),
      .groups = 'drop'
    ) %>%
