...
simulate_market <- function(data_market, coef_demand) {
  ...
  profits <- data_market %>%
    mutate(
      quantity = case_when(
        dummy_leader == 1 ~ Q_leader_total * (qe_ymc / sum(leaders$qe_ymc, na.rm = TRUE)),
        dummy_follower == 1 ~ followers$q_follower[match(id, followers$id)],
        TRUE ~ 0
      ),
      profit = quantity * (price - mc_ymc_hat)
    ) %>%
    select(date, country, id, dummy_leader, quantity, profit)
